...
app.get('/callback', async (req, res) => {
    ...
    try {
        ...
        // Securely set tokens as HTTP-only cookies
        res.cookie('spotify_access_token', access_token, {
            httpOnly: true,
            secure: isProduction,
            sameSite: 'Lax',
            maxAge: expires_in * 1000, // Token expiry (in milliseconds)
        });
        ...
        res.cookie('spotify_refresh_token', refresh_token, {
            ...
            secure: isProduction,
            sameSite: 'Lax',
            maxAge: 30 * 24 * 60 * 60 * 1000, // 30 days expiry for refresh token
        });
...
app.get('/refresh-token', async (req, res) => {
    const refresh_token = req.cookies.spotify_refresh_token; // Get the refresh token from cookies
    const isProduction = process.env.NODE_ENV === 'production';
...
app.get('/spotify-data', async (req, res) => {
    ...
    try {
        // Request user data from Spotify API
